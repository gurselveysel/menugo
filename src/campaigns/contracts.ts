...
export function campaignRequest(value:unknown){
 ...
 if(Object.keys(v).some(k=>!['productId','jobId','expectedVersion','format'].includes(k))||typeof v.productId!=='string'||!UUID.test(v.productId)
  ...
  ||typeof v.expectedVersion!=='string'||!/^[a-f0-9]{64}$/.test(v.expectedVersion)||!['post','story'].includes(String(v.format)))throw Error('INVALID_CAMPAIGN_REQUEST');
...
export function readCampaign(value:unknown):CampaignSnapshot{
 ...
 if(!String(v.name).trim()||typeof v.productId!=='string'||!UUID.test(v.productId)||typeof v.version!=='string'||!/^[a-f0-9]{64}$/.test(v.version)
