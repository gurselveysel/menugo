...
User requires no paid AI services, no automatic credit purchases and no paid fallback. This revision adds a task/privacy/quota-gated router and owner-only multi-provider configuration. It does not activate a subscription, open new infrastructure, modify customer prices or infer successful real-model acceptance from mocked tests.
...
## Providers reviewed (official sources, 2026-09-19)
...
| Groq | Free-account text/JSON route with OpenAI gpt-oss-20b/120b. For private data, owner must enable ZDR before approving this route. | https://console.groq.com/docs/billing-faqs and https://console.groq.com/docs/your-data and https://console.groq.com/docs/structured-outputs |
...
| Cloudflare Workers AI | Separate Free-plan image adapter; FLUX.2 Klein 4B image editing, multipart reference under 512 pixels. Free quota exhaustion must reject rather than upgrade. | https://developers.cloudflare.com/workers-ai/platform/pricing/ and https://developers.cloudflare.com/workers-ai/models/flux-2-klein-4b/ |
...
| Mistral | Experimental/limited free offer not silently treated as production permission; not enabled. | https://docs.mistral.ai/deployment/ai-studio/tier/ |
...
For Gemini/Groq/Cloudflare, a generation key alone cannot attest the account's billing plan. The owner must use a dedicated Free/unbilled account, explicitly attest billing is disabled, and renew that declaration every 30 days. The app blocks after expiry and never enables billing. If the owner upgrades the provider outside MenuGO, disable its MenuGO connection first. App request limits are additional protection, not a substitute for the provider's hard free-plan cap.

## Privacy
- Only owner edits keys; manager can view sanitized configuration. Secrets in Supabase Vault, never returned in status/client storage or committed to Git.
...
- Groq private workloads require ZDR owner confirmation; OpenRouter uses hard ZDR+deny filters. If no private-capable route exists, stop.
...
2. Apply additive ops migration (no catalogue/customer/payment writes).
...
6. Owner enters Free provider keys only through authenticated management UI. Run explicit synthetic non-private probe then a real controlled menu/photo acceptance case before commercial AI use.
