...
BEGIN
 ...
 IF p_action<>'status' THEN
  ...
  IF p_action='save' THEN
   IF p_payload->>'consent' IS DISTINCT FROM 'true' OR p_payload->>'provider' NOT IN('openai','gemini','self_hosted')
    OR NOT ((p_payload->>'provider'='openai' AND p_payload->>'model' IN('gpt-4.1-mini','gpt-4.1')) OR (p_payload->>'provider'='gemini' AND p_payload->>'model'='gemini-2.5-flash') OR (p_payload->>'provider'='self_hosted' AND p_payload->>'model' IN('qwen3.5:4b','qwen3.5:9b','Qwen/Qwen3.5-4B','Qwen/Qwen3.5-9B')))
...
BEGIN
 ...
 IF c.provider<>'self_hosted' THEN RAISE SQLSTATE 'PT409' USING MESSAGE='LLM_OPEN_SOURCE_REQUIRED';END IF;
