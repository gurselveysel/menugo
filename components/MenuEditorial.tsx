...
export function EditorialProducts<T extends EditorialProduct>({items,onSelect,english,name}: {items:readonly T[];onSelect:(item:T)=>void;english:boolean;name:(item:T)=>string}) {
 ...
 return <section className="editorial-picks" aria-labelledby="editorial-picks-title">
  ...
  <p className="editorial-art-note">{english?'Presentation images are illustrative. See each item for its ingredients and current price.':'Sunum görselleri temsilidir. İçerik ve güncel fiyat için ürünü inceleyin.'}</p>
 </section>;
}
