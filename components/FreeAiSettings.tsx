...
const options={
 ...
 cloudflare_free:{name:'Cloudflare Workers Free · Görsel',models:['@cf/black-forest-labs/flux-2-klein-4b'],note:'Ücretsiz Workers planındaki kota kullanılır. Ücretli Workers hesabını bağlamayın. Ürün fotoğrafı referansı 511 piksele küçültülür; orijinal korunur ve çıktı incelemeye sunulur.',url:'https://dash.cloudflare.com/profile/api-tokens'}
} as const;
...
export function FreeAiSettings({onSaved,businessId,branchId}:{onSaved:()=>void;businessId:string;branchId:string}){
 ...
 {provider!=='gemini_free'&&<label className="llm-check"><input name="private" type="checkbox" defaultChecked={r?.privateContentAllowed??false}/>Özel belge, fotoğraf ve sorular için bu sağlayıcıya veri aktarımını onaylıyorum. Groq için ZDR’yi açtım; OpenRouter’da ZDR filtresi zorunludur.</label>}
