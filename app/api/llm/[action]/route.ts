...
import {llmStatus,invokeLlm} from '@/lib/llm/client';
...
export async function POST(req:Request,{params}:{params:Promise<{action:string}>}){try{
 ...
 if(action==='settings'){
  ...
  if(!['openai','gemini'].includes(String(v.provider))||typeof v.model!=='string'||!(llmModels as Record<string,readonly (readonly string[])[]>)[String(v.provider)]?.some(x=>x[0]===v.model)||!Number.isInteger(v.dailyLimit)||Number(v.dailyLimit)<1||Number(v.dailyLimit)>100||typeof v.enabled!=='boolean'||v.consent!==true)throw new Failure('INVALID_LLM_SETTINGS');
 ...
 if(action==='test')return json(await invokeLlm(s,uuid(v.operationId),'test',{}));
 if(action==='ask'){
