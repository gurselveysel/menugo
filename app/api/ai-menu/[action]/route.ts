...
import {requireOpenSource} from '@/lib/llm/client';
...
export async function POST(req:Request,{params}:{params:Promise<{action:string}>}){try{
 ...
 if(action==='upload'){
  ...
  {try{const meta=await sharp(b,{limitInputPixels:40000000}).metadata();if(!meta.width||!meta.height||meta.width*meta.height>40000000)throw new ImportError('UNSUPPORTED_FILE');}catch{throw new ImportError('UNSUPPORTED_FILE');}}
