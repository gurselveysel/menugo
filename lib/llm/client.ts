...
export async function invokeLlm(s:Awaited<ReturnType<typeof client>>,operationId:string,kind:'test'|'assistant'|'menu-extract'|'studio',input:Record<string,unknown>,target:{p_business_id:string;p_branch_id:string}=scope){
 ...
 try{response=await fetch(base.replace(/\/$/,'')+'/functions/v1/menugo-llm',{method:'POST',headers:{Authorization:'Bearer '+data.session.access_token,apikey:process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY!,'Content-Type':'application/json'},body:JSON.stringify({businessId:target.p_business_id,branchId:target.p_branch_id,operationId,kind,input}),signal:AbortSignal.timeout(65000),cache:'no-store',redirect:'error'});}catch{throw new Failure('LLM_RESULT_UNKNOWN',503);}
 ...
 try{for(;;){const r=await reader.read();if(r.done)break;length+=r.value.length;if(length>6500000){await reader.cancel();throw new Failure('LLM_INVALID_OUTPUT',502);}parts.push(r.value);}const value=JSON.parse(Buffer.concat(parts).toString('utf8'));if(!response.ok)throw new Failure(/^[A-Z0-9_]{1,80}$/.test(value?.error?.code)?value.error.code:'LLM_CONNECTION_UNAVAILABLE',response.status);return value;}catch(e){if(e instanceof Failure)throw e;throw new Failure('LLM_RESULT_UNKNOWN',503);}
}
...
export async function requireOpenSource(s:Awaited<ReturnType<typeof client>>,kind?:string){
 ...
 if(status.provider!=='free_router')throw new Failure('LLM_PAID_PROVIDER_BLOCKED',409);
 if(!status.enabled)throw new Failure('LLM_DISABLED',409);
 if(kind==='photo-enhance'&&!status.imageConfigured)throw new Failure('LLM_IMAGE_ENGINE_REQUIRED',409);
