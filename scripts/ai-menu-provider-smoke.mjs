// One explicitly triggered, bounded deployment acceptance; never a public endpoint.
// Synthetic data only. No restaurant/customer records or source documents are read.
...
const report={test:'r15-synthetic-menu',realProviderRequests:0,customerDataSent:false,databaseWrites:false,creditsPurchased:false,checks:[]};
...
try{
 ...
 // Never buy credit or enable auto-recharge. At most two small, non-retried calls.
 ...
 let provider=readFileSync('lib/ai-menu/provider.ts','utf8').replace(/import\s+['"]server-only['"];?/g,'').replace(/from ['"]\.\/contracts['"]/g,"from './contracts.mjs'").replace(/import\s*\{getVercelOidcToken\}\s*from\s*['"]@vercel\/oidc['"];?/,"const getVercelOidcToken=async()=>process.env.VERCEL_OIDC_TOKEN;");
 ...
 for(const [label,bytes,mime]of[['image',image,'image/png'],['pdf',pdf,'application/pdf']]){
  ...
  if(JSON.stringify(values)!==JSON.stringify(['2550','8000']))throw Error('EXTRACTION_MISMATCH_'+label);
...
finally{if(dir)rmSync(dir,{recursive:true,force:true});console.log('AI_PROVIDER_ACCEPTANCE '+JSON.stringify(report));}
