...
try{
 ...
 configured=false;await page.getByRole('button',{name:'Yenile',exact:true}).last().click();await page.getByText('AI bağlantısı henüz yapılandırılmamış.',{exact:false}).waitFor();checks.push('missing provider produces honest unavailable state');
