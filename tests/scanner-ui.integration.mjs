// Actual QR decoding, virtual camera only. API calls are isolated fixtures, not production.
...
async function session(mode='deny',width=390){
 ...
 const page=await context.newPage();page.setDefaultTimeout(15000);page.on('pageerror',e=>errors.push(e.message));return{context,page,metrics};
}
try{
 ...
 for(const width of[320,390,768,1440]){
  const{page,metrics}=await session('deny',width);await page.goto(base+'/siparis',{waitUntil:'networkidle'});
 ...
 {
  const{page,metrics}=await session('qr');await page.goto(base+'/siparis',{waitUntil:'networkidle'});await page.getByRole('button',{name:'Kamerayı aç ve masa QR’sini tara'}).click();
 ...
 {
  const{page,metrics}=await session('blank');await page.goto(base+'/siparis',{waitUntil:'networkidle'});await page.getByRole('button',{name:'Kamerayı aç ve masa QR’sini tara'}).click();await page.getByRole('button',{name:'Kamerayı durdur'}).waitFor();await page.getByRole('button',{name:'Kamerayı durdur'}).click();assert.ok(metrics.stops>=1);checks.push('manual pause stops tracks');await page.getByLabel('QR tarayıcıyı kapat').click();
 }
 {
  const{page,metrics}=await session('late');await page.goto(base+'/siparis',{waitUntil:'networkidle'});await page.getByRole('button',{name:'Kamerayı aç ve masa QR’sini tara'}).click();await page.waitForFunction(()=>typeof window.resolveQrCamera==='function');await page.getByLabel('QR tarayıcıyı kapat').click();await page.evaluate(()=>window.resolveQrCamera());await page.waitForTimeout(150);assert.ok(metrics.stops>=1);checks.push('late permission grant stopped after close');
 }
 {
