export async function assistantCommandChecks({q,db,auth,check,rejects,b,br,users,op,p1}){
 ...
 const beforePrice=(await q('select approved_price::text price,available,updated_at::text updated from public.menu_items where id=$1',[p1]))[0];
 ...
 const afterApply=(await q('select approved_price::text price,available from public.menu_items where id=$1',[p1]))[0];check('assistant command never changes price',afterApply.price===beforePrice.price&&afterApply.available===!product.available);
 ...
 const afterUndo=(await q('select approved_price::text price,available from public.menu_items where id=$1',[p1]))[0];check('undo preserves exact price and restores state',afterUndo.price===beforePrice.price&&afterUndo.available===beforePrice.available);
