...
try{
 ...
 await q('update ops.branch_settings set ordering_enabled=true,dine_in_enabled=true where branch_id=$1',[br]);await q("update ops.service_controls set guest_entry_mode='staff_approved' where branch_id=$1",[br]);await q("insert into ops.dining_tables(id,business_id,branch_id,table_code,display_name)values($1,$2,$3,'14','Masa 14')",[table,b,br]);await auth();const invitation=(await q('select ops.start_table($1,$2,$3) as j',[b,br,table]))[0].j;const cid=invitation.checkId;await anon();
