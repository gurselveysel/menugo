...
const base='http://localhost:3417',checks=[],errors=[],writes=[],out='public/qa/studio';let browser,role='owner',ready=false,approved=false;
...
try{
 ...
 await context.route('**/api/**',async route=>{const req=route.request(),path=new URL(req.url()).pathname;let value={};
  ...
  else if(path==='/api/studio/list')value={jobs:writes.some(x=>x.path==='/api/studio/create')?[{id:jid,kind:'product-copy',state:approved?'approved':'review'}]:[],catalogue:[item],aiReady:ready,aiReason:ready?null:'AI_CREDIT_REQUIRED'};
  else if(path==='/api/studio/get')value={...result,state:approved?'approved':'review'};
  else if(path==='/api/studio/create'){assert.equal(req.postDataJSON().kind,'product-copy');value={id:jid,state:'queued'};}
 ...
 ready=true;await page.getByRole('button',{name:'İçerik & Görsel',exact:true}).click();await page.getByRole('button',{name:'Listeyi yenile'}).click();await page.getByLabel('Ne hazırlayalım?').selectOption('product-copy');await page.getByRole('button',{name:'Taslak üret',exact:true}).click();await page.getByRole('heading',{name:'Peynirli Börek',exact:true}).waitFor();checks.push('server draft and source reference rendered');
 ...
 assert.deepEqual(errors,[]);fs.writeFileSync(out+'/results.json',JSON.stringify({passed:checks.length,checks,errors,apiDoubles:true,realProviderCalls:0,productionTransactions:0},null,2));console.log('STUDIO UI PASS',checks.length);
